...
const CATEGORY_CONFIG: Record<SkillCategory, { label: string; icon: React.ReactNode; bg: string; text: string }> = {
...
function parseFrontmatter(raw: string): { frontmatter: Frontmatter; body: string } {
  ...
  for (const line of match[1].split('\n')) {
    ...
    if (val.startsWith('[') && val.endsWith(']')) {
      fm[key.trim()] = val.slice(1, -1).split(',').map((s) => s.trim().replace(/^['"]|['"]$/g, ''))
    } else {
...
function SidePanel({ skill, agentId, allAgentGroups, onClose, onSave, onAssign }: SidePanelProps) {
  ...
  const { frontmatter, body } = skill.instructions
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800 px-5">
        {([
          ...
          { id: 'skill-md', label: 'SKILL.md', icon: <ScrollText className="w-3 h-3" /> },
        ] as const).map((tab) => (
          <button
            key={tab.id}
                ...
                <div className="space-y-1">
                  {Object.entries(frontmatter).map(([k, v]) => (
                    <div key={k} className="flex gap-2 text-[11px]">
                      <span className="text-slate-400 dark:text-slate-500 shrink-0" style={{ fontFamily: '"JetBrains Mono", monospace' }}>{k}:</span>
                      <span className="text-slate-700 dark:text-slate-300" style={{ fontFamily: '"JetBrains Mono", monospace' }}>
